...
export const StudentCardGenerator = () => {
  ...
  const { settings, getSchoolInfo } = useSettings();
  ...
  const getStudentFaculty = (student: Student) => {
    if (student.enrollments && student.enrollments.length > 0) {
      ...
      const faculty = classes.find(
        (f) => f.id === latestEnrollment.schoolClass.id,
      );
  ...
  const getStudentLevel = (student: Student) => {
    if (student.enrollments && student.enrollments.length > 0) {
      const latestEnrollment =
        student.enrollments[student.enrollments.length - 1];
      return latestEnrollment.schoolClass.level || "Non assigné";
    }
    return "Non assigné";
  };
  ...
  const getStudentFacultyId = (student: Student) => {
    if (student.enrollments && student.enrollments.length > 0) {
      const latestEnrollment =
        student.enrollments[student.enrollments.length - 1];
      return latestEnrollment.schoolClass.id || "";
    }
    return "";
  };
  ...
  const exportAsPNG = async () => {
    if (frontCardRef.current && backCardRef.current && !isExporting) {
      ...
      try {
        ...
        frontLink.download = `carte-etudiant-${selectedStudentData?.studentCode}-avant.png`;
        ...
        backLink.download = `carte-etudiant-${selectedStudentData?.studentCode}-arriere.png`;
  ...
  const generateQRCode = (studentData: Student) => {
    ...
    const data = `${studentData.studentCode}|${studentData.firstName}|${studentData.lastName}|${faculty}|${level}|${academicYear}`;
              ...
              >
                ...
                >
                  ...
                  <div className="flex items-center justify-center border border-orange-500 rounded-tl-xl rounded-br-xl mx-1 mt-3 px-0 py-1 pt-1 backdrop-blur-sm">
                    <div className="w-16 h-14 flex items-center justify-center mr-0 pl-1">
                      <img
                        src="/logo.png"
                        alt="Logo UJEPH"
                    ...
                    <div className="text-center text-white">
                      <p className="text-xs font-bold leading-tight">
                        {schoolInfo.slogan ||
                          "Université de l'Excellence en Haïti"}
                      </p>
                      <p className="text-[9px]">
                        {schoolInfo.name || "L'excellence pour tous"}
                      </p>
                      <p className="text-[9px]">
                  ...
                  <div className="mt-4 mx-10 font-semibold text-black text-[14px] space-y-1 leading-3 mb-4">
                    <p className="mb-0 ">
                      <strong>Code: </strong> {selectedStudentData.studentCode}
                    </p>
                    ...
                    <p className="mb-16 leading-3">
                      ...
                      {getStudentLevel(selectedStudentData)}
                    </p>
                  </div>
                ...
                >
                  ...
                  <div className="mt-2 pr-3 pt-4 pl-3 bg-black/20 rounded-lg">
                    <p className="text-xs text-center">
                      Cette carte est la propriété de l'
                      {schoolInfo.slogan || "de l'Excellence en Haïti"}. Toute
                      falsification est passible de poursuites.
                    </p>
                  </div>
                  ...
                  <div className="flex justify-between items-center mt-4 text-xs opacity-70">
                    <span>
                      ...
                      {schoolInfo.name || "Université de l'Excellence en Haïti"}
                    </span>
                    <span>Code: {selectedStudentData.studentCode}</span>
                  </div>
                </div>
              </motion.div>
            </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {student.studentCode} • {getStudentFaculty(student)} •
