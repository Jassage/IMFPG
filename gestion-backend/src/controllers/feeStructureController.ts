// controllers/feeStructureController.ts
...
export const createFeeStructure = async (req: Request, res: Response) => {
  try {
    const { name, academicYear, amount, description, dueDate, isActive } =
      req.body;
...
export const deleteFeeStructure = async (req: Request, res: Response) => {
  try {
    ...
    if (feeStructure.studentFees.length > 0) {
      const studentsWithPayments = feeStructure.studentFees.filter(
      ...
      if (studentsWithPayments.length > 0) {
        return res.status(400).json({
          error:
            "Impossible de supprimer cette structure car des paiements y sont associés",
          details: {
...
export const forceDeleteFeeStructure = async (req: Request, res: Response) => {
  try {
    ...
    if (hasPayments) {
      return res.status(400).json({
        error:
          "Impossible de supprimer: des paiements sont associés à cette structure",
        details: {
