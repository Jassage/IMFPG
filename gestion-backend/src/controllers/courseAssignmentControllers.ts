...
interface CourseAssignmentCreateData {
  ueId: string;
  professeurId: string;
  facultyId: string;
  level: string;
...
interface CourseAssignmentUpdateData {
  ueId?: string;
  professeurId?: string;
  facultyId?: string;
  level?: string;
...
function validateAssignmentInput(data: any): {
  isValid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
...
export const createCourseAssignment = async (req: Request, res: Response) => {
  ...
  try {
    const assignmentData: CourseAssignmentCreateData = req.body;
...
export const getCourseAssignments = async (req: Request, res: Response) => {
  ...
  try {
    const { professorId, facultyId, academicYearId, semester, level, ueId } =
      req.query;

    const where: any = {};

    // Filtres avec validation
    if (professorId) where.professeurId = professorId as string;
    if (facultyId) where.facultyId = facultyId as string;
    if (academicYearId) where.academicYearId = academicYearId as string;
    if (semester) {
      if (["S1", "S2"].includes(semester as string)) {
        where.semester = semester as "S1" | "S2";
      } else {
        await createAuditLog({
          ...auditData,
          ...
          entity: "CourseAssignment",
          description:
            "Tentative de récupération des affectations - semestre invalide",
          status: "ERROR",
        });
        return res.status(400).json({
          success: false,
    ...
    if (level) {
      if (["1", "2", "3", "4", "5"].includes(level as string)) {
        ...
        await createAuditLog({
          ...
          entity: "CourseAssignment",
          description:
            "Tentative de récupération des affectations - niveau invalide",
          status: "ERROR",
        });
        return res.status(400).json({
          success: false,
    ...
    if (ueId) where.ueId = ueId as string;
...
export const updateCourseAssignment = async (req: Request, res: Response) => {
  ...
  try {
    const { id } = req.params;
    const updateData: CourseAssignmentUpdateData = req.body;
    ...
    const modifiedFields = Object.keys(updateData).filter(
      (key) => updateData[key as keyof CourseAssignmentUpdateData] !== undefined
    );
    ...
    const modifiedKeyFields = modifiedFields.filter((field) =>
...
export const getAssignmentsByFaculty = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (level) {
      if (["1", "2", "3", "4", "5"].includes(level as string)) {
        ...
        await createAuditLog({
          ...
          entity: "CourseAssignment",
          description:
            "Tentative de récupération par faculté - niveau invalide",
          status: "ERROR",
        });
        return res.status(400).json({
          success: false,
    ...
    if (semester) {
      if (["S1", "S2"].includes(semester as string)) {
        ...
        await createAuditLog({
          ...
          entity: "CourseAssignment",
          description:
            "Tentative de récupération par faculté - semestre invalide",
          status: "ERROR",
        });
        return res.status(400).json({
          success: false,
    ...
    handleControllerError(
      error,
      res,
      "la récupération des affectations par faculté",
      {
        ...auditData,
...
export const getUEsByFacultyAndLevel = async (req: Request, res: Response) => {
  ...
  try {
    ...
    handleControllerError(
      error,
      res,
      "la récupération des Cours par faculté et niveau",
      {
        ...auditData,
  ...
  try {
    ...
    if (!professorId) {
      await createAuditLog({
        ...
        entity: "CourseAssignment",
        description:
          "Tentative de récupération des affectations du professeur - ID manquant",
        status: "ERROR",
      });
      return res.status(400).json({
        success: false,
    ...
    const where: any = { professeurId: professorId };
    ...
    if (semester) {
      if (["S1", "S2"].includes(semester as string)) {
        ...
        await createAuditLog({
          ...
          entity: "CourseAssignment",
          description:
            "Tentative de récupération des affectations du professeur - semestre invalide",
          status: "ERROR",
        });
        return res.status(400).json({
          success: false,
    ...
    handleControllerError(
      error,
      res,
      "la récupération des affectations du professeur",
      {
        ...auditData,
...
export const copyAssignments = async (req: Request, res: Response) => {
  const auditData = {
    ipAddress: req.ip || "unknown",
    userAgent: req.get("User-Agent") || "unknown",
    userId: (req as any).userId || "unknown",
  ...
  try {
    const {
      ...
      copyProfessors = true, // Garder les mêmes professeurs
...
const resolveEntitiesFromNames = async (
  assignments: ExcelAssignmentRow[]
): Promise<{
  processed: ProcessedAssignment[];
}> => {
  const processed: ProcessedAssignment[] = [...assignments];
  ...
  console.log(
    "📦 TOUTES les facultés disponibles:",
    faculties.map((f) => ({ name: f.name, id: f.id }))
  );
  ...
  const findProfessor = (professorName: string) => {
    const cleanName = professorName.toLowerCase().trim().replace(/\s+/g, " ");

    // Recherche exacte
    const exactMatch = allProfessors.find(
      (p) =>
    ...
    if (exactMatch) {
      console.log(
        `🔍 Professeur trouvé (exact): ${professorName} -> ${exactMatch.firstName} ${exactMatch.lastName}`
      );
      return exactMatch;
    }

    // Recherche par nom de famille seulement
    const lastNameMatch = allProfessors.find(
      (p) => p.lastName.toLowerCase().trim() === cleanName
    );
    if (lastNameMatch) {
      console.log(
        `🔍 Professeur trouvé (nom seul): ${professorName} -> ${lastNameMatch.firstName} ${lastNameMatch.lastName}`
      );
    ...
    const flexibleMatch = allProfessors.find((p) => {
      const profFullName = `${p.firstName} ${p.lastName}`
        ...
        .replace(/\s+/g, " ");
      return (
        cleanName.includes(p.lastName.toLowerCase()) ||
    ...
    if (flexibleMatch) {
      console.log(
        `🔍 Professeur trouvé (flexible): ${professorName} -> ${flexibleMatch.firstName} ${flexibleMatch.lastName}`
      );
    }
  ...
  const findFaculty = (facultyName: string) => {
    ...
    if (faculty) {
      console.log(
        `✅ Faculté trouvée (nettoyée): ${facultyName} -> ${faculty.name}`
      );
      return faculty;
    }
    ...
    if (faculty) {
      console.log(
        `✅ Faculté trouvée (originale): ${facultyName} -> ${faculty.name}`
      );
      return faculty;
    }
    ...
    for (const [key, fac] of facultyMap.entries()) {
      if (
        ...
        console.log(
          `✅ Faculté trouvée (flexible): ${facultyName} -> ${fac.name}`
        );
        return fac;
      }
    }
  ...
  for (const assignment of processed) {
    ...
    if (professor) {
      ...
      console.log(
        `✅ Professeur résolu: ${assignment.nomProfesseur} -> ${professor.id}`
      );
    } else {
    ...
    if (faculty) {
      ...
      console.log(
        `✅ Faculté résolue: ${assignment.nomFaculte} -> ${faculty.id}`
      );
    } else {
    ...
    if (academicYear) {
      ...
      console.log(
        `✅ Année académique trouvée: ${assignment.anneeAcademique} -> ${academicYear.id}`
      );
    } else {
      errors.push(
        `Année académique non trouvée: ${assignment.anneeAcademique}`
      );
      console.log(
        `❌ Année académique non trouvée: ${assignment.anneeAcademique}`
      );
...
export const importAssignmentsFromExcel = async (
  req: Request,
  res: Response
) => {
  const auditData = {
  ...
  try {
    ...
    const excelAssignments: ExcelAssignmentRow[] = jsonData
      .filter(
        (row: any) =>
          row.codeUE &&
          row.nomProfesseur &&
    ...
    const { processed: assignmentsWithIds } =
      await resolveEntitiesFromNames(excelAssignments);
    ...
    for (const [index, assignment] of assignmentsWithIds.entries()) {
      try {
        ...
        const { ueId, professeurId, facultyId, academicYearId } =
          assignment.resolvedIds;
...
export const downloadAssignmentTemplate = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=template_affectations.xlsx"
    );
