...
export const getProfesseurFullDetailsService = async (id: string) => {
  ...
  const totalSubjects = new Set(assignments.map((a: any) => a.subjectId)).size;
...
export const createProfesseurService = async (data: CreateProfesseurData) => {
  ...
  if (createUserAccount && !finalUserId) {
    try {
      const userResult = await createUserForProfesseur(
        email,
        firstName,
        lastName,
        sendInvitation
  ...
  if (matricule && matricule.trim()) createData.matricule = matricule.trim();
...
export const attachUserToProfesseurService = async (
  ...
  if (email && !userId) {
    ...
    if (existingUser) {
      ...
      // Créer un nouvel utilisateur
      const result = await createUserForProfesseur(
        email,
