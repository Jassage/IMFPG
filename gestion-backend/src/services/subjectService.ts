...
export class SubjectService {
   ...
   */
  async getSubjectById(id: string, auditData: AuditData) {
    try {
  ...
  async updateSubject(
    ...
    try {
      ...
      const validSubjectTypes: SubjectType[] = [
        "Obligatoire" as SubjectType,
        "Optionnelle" as SubjectType,
      ];
   ...
   */
  async deleteSubject(id: string, auditData: AuditData) {
    try {
      // Vérifier si la matière existe
      const subject = await prisma.subject.findUnique({
        where: { id },
