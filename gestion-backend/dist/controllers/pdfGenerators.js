...
// En-tête universitaire commun
function generateUniversityHeader(doc) {
    // Rectangle de fond
...
function generateBulletinPDF(doc, options) {
    ...
    // Sous-titre
    doc.setFontSize(12);
    doc.text(`Année académique ${academicYear.year} - ${getSemesterLabel(semester)}`, 105, 62, { align: "center" });
    let currentY = 75;
...
function generateRelevePDF(doc, options) {
    ...
    doc.text(`Date: ${new Date().toLocaleDateString("fr-FR")}`, 120, currentY + 10);
    currentY += 20;
...
function generateAttestationNiveauPDF(doc, options) {
    ...
    currentY += 15;
    // Signature et cachet
    generateFormalSignatureSection(doc, currentY, "Le Directeur des Études", "Director of Studies");
    return Buffer.from(doc.output("arraybuffer"));
}
// 4. ATTESTATION DE FIN D'ÉTUDES
function generateAttestationFinEtudesPDF(doc, options) {
    const { student, academicYear, level, language = "FR", statistics = {}, } = options;
    // Titre
    doc.setFontSize(16);
    doc.setTextColor(...COLORS.DARK);
    doc.setFont("helvetica", "bold");
...
function generateStudentInfoSection(doc, student, level, startY) {
    ...
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(`Nom et prénom: ${student.lastName} ${student.firstName}`, 20, startY + 8);
...
function generateStatisticsSection(doc, statistics, startY) {
    doc.setFillColor(...COLORS.LIGHT_GRAY);
...
function generateSignatureSection(doc, startY, withSignature, withStamp) {
    ...
    doc.setFontSize(8);
    doc.setTextColor(...COLORS.DARK);
    doc.text("Daniel 2:17 - C'est Dieu qui change les temps et les circonstances", 105, pageHeight - 15, { align: "center" });
    doc.text("Document officiel - Ne pas jeter", 105, pageHeight - 10, {
        align: "center",
    });
}
function generateFormalSignatureSection(doc, startY, titleFr, titleEn) {
...
function generateDiplomaSignatureSection(doc, startY) {
...
function generateAdministrativeSignatureSection(doc, startY) {
    const signatureY = startY;
    doc.setFontSize(10);
...
function calculateSuccessRate(statistics) {
    if (!statistics.creditsEarned ||
        !statistics.totalCredits ||
        statistics.totalCredits === 0) {
        return 0.0;
    }
    return parseFloat(((statistics.creditsEarned / statistics.totalCredits) * 100).toFixed(1));
}
//# sourceMappingURL=pdfGenerators.js.map