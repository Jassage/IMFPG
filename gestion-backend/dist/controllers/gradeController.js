...
exports.downloadGradeTemplate = exports.importGradesFromExcel = exports.getGradeHistory = exports.bulkCreateGrades = exports.getUEGrades = exports.getStudentGrades = exports.deleteGrade = exports.updateGrade = exports.createGrade = exports.getGradeById = exports.getAllGrades = void 0;
...
function calculateGradeStatus(grade, passingGrade) {
    if (grade >= passingGrade) {
        return prisma_2.GradeStatus.Valid_;
    }
    else if (grade >= passingGrade * 0.7) {
        return prisma_2.GradeStatus.reprendre;
    }
    else {
        return prisma_2.GradeStatus.Non_valid_;
    }
}
function validateGradeInput(grade) {
...
function validateSession(session) {
    const validSessions = Object.values(prisma_2.GradeSession);
    return validSessions.includes(session);
}
function validateStatus(status) {
    const validStatuses = Object.values(prisma_2.GradeStatus);
    return validStatuses.includes(status);
}
// ==================== MIDDLEWARE DE VALIDATION ====================
const validateGradeCreation = (data) => {
    const errors = [];
...
const getAllGrades = async (req, res) => {
    ...
    try {
        const { studentId, ueId, academicYearId, semester, level, session, status, } = req.query;
        ...
        if (session) {
            if (validateSession(session)) {
                ...
                return res.status(400).json({
                    ...
                    validSessions: Object.values(prisma_2.GradeSession),
                });
            }
        }
        if (status) {
            if (validateStatus(status)) {
                ...
                return res.status(400).json({
                    ...
                    validStatuses: Object.values(prisma_2.GradeStatus),
                });
            }
        }
...
const getGradeById = async (req, res) => {
    ...
    try {
        ...
        const grade = await prisma_1.default.grade.findFirst({
            where: {
                id,
                isActive: true,
            },
...
const createGrade = async (req, res) => {
    ...
    try {
        ...
        const [student, ue, academicYear] = await Promise.all([
            prisma_1.default.student.findUnique({ where: { id: gradeData.studentId } }),
            prisma_1.default.ue.findUnique({
                where: { id: gradeData.ueId },
                select: { id: true, passingGrade: true, code: true, title: true },
            }),
            prisma_1.default.academicYear.findUnique({
        ...
        const finalSession = gradeData.session || prisma_2.GradeSession.Normale;
        ...
        const existingGrade = await prisma_1.default.grade.findFirst({
            where: {
                ...
                isActive: finalSession === prisma_2.GradeSession.Normale ? true : undefined,
            },
        });
        if (existingGrade) {
            console.log("⚠️ Grade already exists:", existingGrade);
            if (finalSession === prisma_2.GradeSession.Normale) {
                ...
                if (existingGrade.isActive === false) {
                    ...
                    const updatedGrade = await prisma_1.default.grade.update({
                        ...
                        data: {
                            ...
                            session: prisma_2.GradeSession.Normale,
                            isActive: true,
                            updatedAt: new Date(),
                        },
                        include: {
        ...
        const finalStatus = gradeData.status || calculateGradeStatus(numericGrade, ue.passingGrade);
...
const updateGrade = async (req, res) => {
    ...
    try {
        ...
        const existingGrade = await prisma_1.default.grade.findUnique({
            where: { id },
            include: {
                ue: {
                    select: {
        ...
        if (updateData.isRetake) {
            ...
            const existingRetake = await prisma_1.default.grade.findFirst({
                where: {
            ...
            const newGradeValue = updateData.grade !== undefined
                ? Number(updateData.grade)
                : existingGrade.grade;
            const newStatus = calculateGradeStatus(newGradeValue, existingGrade.ue.passingGrade);
            // Créer une nouvelle note pour la reprise
            const retakeGrade = await prisma_1.default.grade.create({
                data: {
                    studentId: existingGrade.studentId,
                    ueId: existingGrade.ueId,
                    grade: newGradeValue,
        ...
        if (updateData.grade !== undefined) {
            updatePayload.status = calculateGradeStatus(Number(updateData.grade), existingGrade.ue.passingGrade);
        }
    ...
    try {
        ...
        if (!studentId) {
            await (0, auditController_1.createAuditLog)({
                ...
                entity: "Grade",
                description: "Tentative de récupération des notes d'étudiant - ID manquant",
                status: "ERROR",
            });
        ...
        handleControllerError(error, res, "la récupération des notes de l'étudiant", {
            ...auditData,
...
const getUEGrades = async (req, res) => {
    const auditData = {
        ipAddress: req.ip || "unknown",
        userAgent: req.get("User-Agent") || "unknown",
        userId: req.userId || "unknown",
    ...
    try {
        ...
        const stats = {
            total: grades.length,
            valid: grades.filter((g) => g.status === prisma_2.GradeStatus.Valid_).length,
            reprendre: grades.filter((g) => g.status === prisma_2.GradeStatus.reprendre)
                .length,
            nonValid: grades.filter((g) => g.status === prisma_2.GradeStatus.Non_valid_)
                .length,
...
exports.getUEGrades = getUEGrades;
            ...
            try {
                ...
                const existingGrade = await prisma_1.default.grade.findFirst({
                    where: {
                        ...
                        academicYearId: gradeData.academicYearId,
                        semester: gradeData.semester,
                        session: gradeData.session || prisma_2.GradeSession.Normale,
                        isActive: true,
                    },
                });
                if (existingGrade) {
    ...
    try {
        ...
        if (!studentId || !ueId || !academicYearId || !semester) {
            await (0, auditController_1.createAuditLog)({
                ...
                entity: "Grade",
                description: "Tentative de consultation d'historique - paramètres manquants",
                status: "ERROR",
            });
            return res.status(400).json({
        ...
        handleControllerError(error, res, "la récupération de l'historique des notes", {
            ...auditData,
...
const resolveEntitiesForGrades = async (grades) => {
    const processed = [...grades];
    ...
    for (const grade of processed) {
        ...
        if (academicYear) {
            ...
            console.log(`✅ Année académique trouvée: ${grade.anneeAcademique} -> ${academicYear.id}`);
        }
        else {
...
const importGradesFromExcel = async (req, res) => {
    const auditData = {
        ipAddress: req.ip || "unknown",
        userAgent: req.get("User-Agent") || "unknown",
        userId: req.user?.id || req.userId || null,
    ...
    try {
        ...
        const excelGrades = jsonData
            .filter((row) => row.matricule &&
            row.codeUE &&
            row.note !== undefined &&
            row.niveau &&
            row.anneeAcademique &&
        ...
        if (excelGrades.length === 0) {
            ...
            await (0, auditController_1.createAuditLog)({
                ...
                action: "IMPORT_GRADES_EXCEL_ATTEMPT",
                entity: "Grade",
                description: "Tentative d'import Excel notes - aucune donnée valide trouvée",
                status: "ERROR",
            });
            return res.status(400).json({
                success: false,
        ...
        const { processed: gradesWithIds } = await resolveEntitiesForGrades(excelGrades);
        ...
        for (const [index, grade] of gradesWithIds.entries()) {
            try {
                ...
                console.log(`➕ Création note: ${grade.matricule} - ${grade.codeUE} - ${grade.note}`);
...
exports.importGradesFromExcel = importGradesFromExcel;
// Template pour l'importation des notes (sans nomProfesseur)
const downloadGradeTemplate = async (req, res) => {
    try {
        const XLSX = await Promise.resolve().then(() => __importStar(require("xlsx")));
        // Créer un workbook
        ...
        res.setHeader("Content-Disposition", "attachment; filename=template_notes.xlsx");
        res.send(buffer);
    }
    catch (error) {
...
exports.downloadGradeTemplate = downloadGradeTemplate;
