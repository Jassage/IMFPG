...
exports.getGuardianStatistics = exports.setPrimaryGuardian = exports.searchGuardians = exports.deleteGuardian = exports.updateGuardian = exports.createGuardian = exports.getGuardianById = exports.getAllGuardians = void 0;
...
const getAllGuardians = async (req, res) => {
    try {
        ...
        const { page = 1, limit = 50, studentId, search, hasParentAccount, isPrimary, } = req.query;
        ...
        const [guardians, total] = await Promise.all([
            prisma_1.default.guardian.findMany({
                where,
                select: {
                    id: true,
                    firstName: true,
            ...
            prisma_1.default.guardian.count({ where }),
        ]);
        console.log(`✅ ${guardians.length} guardians récupérés sur ${total} total`);
        // Formater la réponse
        const response = {
            success: true,
...
const getGuardianById = async (req, res) => {
    try {
        ...
        const guardian = await prisma_1.default.guardian.findUnique({
            where: { id },
            include: {
                student: {
                    select: {
...
const createGuardian = async (req, res) => {
    try {
        const { firstName, lastName, email, phone, address, relationship, isPrimary = false, studentId, parentId, notes, } = req.body;
        ...
        const existingGuardian = await prisma_1.default.guardian.findFirst({
            where: {
                phone,
                studentId,
            },
        ...
        if (existingGuardian) {
            res.status(400).json({
                success: false,
                message: "Un guardian avec ce numéro de téléphone existe déjà pour cet étudiant",
            });
            return;
        }
        ...
        if (parentId) {
            ...
            const existingParentGuardian = await prisma_1.default.guardian.findFirst({
                where: {
                    parentId,
                    studentId,
                },
        ...
        const studentGuardians = await prisma_1.default.guardian.findMany({
            where: { studentId },
        });
        ...
        if (shouldBePrimary && studentGuardians.length > 0) {
            await prisma_1.default.guardian.updateMany({
                where: { studentId },
                data: { isPrimary: false },
            });
        }
        // Créer le guardian
        const guardian = await prisma_1.default.guardian.create({
            data: {
                firstName,
                lastName,
                email: email || null,
...
const updateGuardian = async (req, res) => {
    try {
        ...
        const existingGuardian = await prisma_1.default.guardian.findUnique({
            where: { id },
        });
        if (!existingGuardian) {
            res.status(404).json({
        ...
        if (updateData.phone && updateData.phone !== existingGuardian.phone) {
            const existingPhone = await prisma_1.default.guardian.findFirst({
                where: {
        ...
        const guardian = await prisma_1.default.guardian.update({
            where: { id },
            data: {
...
const deleteGuardian = async (req, res) => {
    try {
        ...
        if (guardian.isPrimary) {
            const otherGuardians = await prisma_1.default.guardian.findMany({
                where: {
            ...
            if (otherGuardians.length > 0) {
                await prisma_1.default.guardian.update({
        ...
        await prisma_1.default.guardian.delete({
            where: { id },
        });
 ...
 * @access Admin/Staff
 */
const searchGuardians = async (req, res) => {
    try {
        const { q, studentId } = req.query;
        console.log(`🔍 GET /api/guardians/search - q: ${q}, studentId: ${studentId}`);
...
exports.searchGuardians = searchGuardians;
/**
 * @desc Définit un guardian comme principal pour un étudiant
 * @route PUT /api/guardians/:id/set-primary
 * @access Admin/Staff
 */
const setPrimaryGuardian = async (req, res) => {
    try {
        const { id } = req.params;
        const { studentId } = req.body;
        console.log(`⭐ PUT /api/guardians/${id}/set-primary pour étudiant ${studentId}`);
        if (!studentId) {
            res.status(400).json({
                success: false,
        ...
        const guardian = await prisma_1.default.guardian.findFirst({
            where: {
                id,
                studentId,
            },
        ...
        const updatedGuardian = await prisma_1.default.guardian.findUnique({
            where: { id },
            include: {
                student: {
                    select: {
...
exports.setPrimaryGuardian = setPrimaryGuardian;
/**
 * @desc Récupère les statistiques des guardians
 * @route GET /api/guardians/statistics
 * @access Admin/Staff
 */
const getGuardianStatistics = async (req, res) => {
    try {
        console.log("📊 GET /api/guardians/statistics");
        // Compter par relation
        const relationshipStats = await prisma_1.default.guardian.groupBy({
            by: ["relationship"],
            _count: {
                id: true,
            },
        ...
        const parentAccountStats = await prisma_1.default.guardian.groupBy({
            by: ["parentId"],
            _count: {
                id: true,
            },
        ...
        const primaryStats = await prisma_1.default.guardian.groupBy({
            by: ["isPrimary"],
            _count: {
                id: true,
            },
        ...
        // Nombre total
        const totalGuardians = await prisma_1.default.guardian.count();
        // Derniers ajouts (7 derniers jours)
        const weekAgo = new Date();
        weekAgo.setDate(weekAgo.getDate() - 7);
        const recentAdditions = await prisma_1.default.guardian.count({
            where: {
                createdAt: {
                    gte: weekAgo,
                },
        ...
        const studentsWithGuardians = await prisma_1.default.student.findMany({
            where: {
                guardians: {
                    some: {},
                },
        ...
        const statistics = {
            ...
            withParentAccount: parentAccountStats.find((item) => item.parentId)?._count.id || 0,
            withoutParentAccount: parentAccountStats.find((item) => !item.parentId)?._count.id || 0,
...
exports.getGuardianStatistics = getGuardianStatistics;
