...
class BulletinService {
    ...
    calculateStatistics(grades) {
        ...
        const weightedAverage = totalCoefficient > 0 ? totalWeightedGrades / totalCoefficient : 0;
     ...
     */
    async recordDocumentAction(transcriptId, action, userId, ipAddress, userAgent) {
     ...
     */
    getControlPeriodDescription(controlType) {
        const periods = {
     ...
     */
    async generateBulletinPDF(bulletinData, options) {
     ...
     */
    generatePDFContent(doc, bulletinData, options) {
        // En-tête
     ...
     */
    generateHeader(doc, bulletinData, options) {
     ...
     */
    generateStudentInfo(doc, bulletinData) {
        doc
            .fontSize(14)
            .font("Helvetica-Bold")
        ...
        doc
            ...
            .text(`Classe: ${bulletinData.classInfo.name} (${bulletinData.classInfo.level})`)
     ...
     */
    generateGradesTable(doc, bulletinData) {
        doc
            .fontSize(14)
            .font("Helvetica-Bold")
        ...
        doc.text("Professeur", 50 + colWidths[0] + colWidths[1] + colWidths[2], tableTop);
        // Ligne de séparation
        doc
        ...
        bulletinData.grades.forEach((grade, index) => {
            ...
            doc.text(grade.professeur, 50 + colWidths[0] + colWidths[1] + colWidths[2], y);
            y += 20;
     ...
     */
    generateStatistics(doc, bulletinData) {
        doc
            .fontSize(14)
            .font("Helvetica-Bold")
        ...
        doc
            .fontSize(10)
            .font("Helvetica")
            .text(`Moyenne générale: ${bulletinData.statistics.weightedAverage.toFixed(2)}/20`)
            .text(`Moyenne pondérée: ${bulletinData.statistics.weightedAverage.toFixed(2)}/20`)
            .text(`Taux de réussite: ${bulletinData.statistics.successRate.toFixed(2)}%`)
            .text(`Total coefficients: ${bulletinData.statistics.totalCoefficient}`);
        if (bulletinData.statistics.minGrade !== undefined &&
            bulletinData.statistics.maxGrade !== undefined) {
            doc
                .text(`Note minimale: ${bulletinData.statistics.minGrade.toFixed(2)}/20`)
                .text(`Note maximale: ${bulletinData.statistics.maxGrade.toFixed(2)}/20`);
        }
        doc.moveDown(1);
    }
    /**
     ...
     */
    generateRemarks(doc, bulletinData) {
        doc
            .fontSize(14)
            .font("Helvetica-Bold")
     ...
     */
    generateFooter(doc, bulletinData) {
        const pageHeight = doc.page.height;
        const footerY = pageHeight - 50;
        doc
            .fontSize(8)
            .font("Helvetica")
            .text(`Document généré le: ${bulletinData.metadata.generatedAt.toLocaleDateString("fr-FR")}`, 50, footerY)
            .text(`Numéro de document: ${bulletinData.metadata.documentNumber}`, 250, footerY, { align: "center" })
            .text("Page 1/1", 450, footerY, { align: "right" });
    }
    /**
     * Enregistre le bulletin dans la base de données
     */
    async saveTranscript(bulletinData, pdfBuffer, generatedBy) {
        try {
            const transcript = await this.prisma.transcript.create({
                data: {
                    ...
                    generatedBy,
                    language: "FR",
                    metadata: bulletinData.metadata,
     ...
     */
    async getStudentTranscripts(studentId, academicYearId) {
        const whereClause = {
            studentId,
            documentType: "BULLETIN",
        };
