...
class AuthService {
     ...
     */
    static async authenticateUser(credentials, ipAddress = "unknown") {
        try {
            ...
            const cleanEmail = (0, validators_1.sanitizeInput)(email).toLowerCase();
            const cleanPassword = (0, validators_1.sanitizeInput)(password);
            ...
            const lockCheck = await this.checkAccountLock(user.id, user.loginAttempts[0]);
            ...
            const isPasswordValid = await (0, security_1.verifyPassword)(cleanPassword, user.password);
            if (!isPasswordValid) {
                const attemptResult = await this.handleFailedLoginAttempt(user.id, user.loginAttempts[0]?.failedAttempts || 0, ipAddress);
            ...
            // Génération du token
            const token = (0, security_1.generateJwtToken)({
                id: user.id,
                email: user.email,
                role: user.role,
    ...
    static async registerUser(userData) {
        try {
            ...
            // Validation avancée
            const validatedRole = (0, validators_1.validateUserRole)(role);
            ...
            if (!(0, validators_1.validatePasswordStrength)(password)) {
                return {
            ...
            const existingUser = await prisma.user.findUnique({
                where: { email: (0, validators_1.sanitizeInput)(email).toLowerCase() },
            });
            if (existingUser) {
                return {
            ...
            const user = await prisma.user.create({
                data: {
                    firstName: (0, validators_1.sanitizeInput)(firstName),
                    lastName: (0, validators_1.sanitizeInput)(lastName),
                    email: (0, validators_1.sanitizeInput)(email).toLowerCase(),
                    phone: phone ? (0, validators_1.sanitizeInput)(phone) : null,
                    role: validatedRole,
     ...
     */
    static async createRoleSpecificProfile(userId, role) {
        try {
            switch (role) {
                case "Professeur":
            ...
            throw new Error("Erreur lors de la création du profil spécifique au rôle");
        }
    }
    /**
     ...
     */
    static async checkAccountLock(userId, latestAttempt) {
        ...
        if (latestAttempt && currentFailedAttempts >= MAX_LOGIN_ATTEMPTS) {
            ...
            if (timeSinceLock < LOCK_TIME_MS) {
                const remainingMinutes = Math.ceil((LOCK_TIME_MS - timeSinceLock) / 60000);
                return {
                    isLocked: true,
                    remainingMinutes,
     ...
     */
    static async handleFailedLoginAttempt(userId, currentAttempts, ipAddress) {
     ...
     * @param {string} userId - ID de l'utilisateur
     */
    static async handleSuccessfulLogin(userId) {
     ...
     * @param {string} userId - ID de l'utilisateur
     */
    static async clearLoginAttempts(userId) {
        try {
    ...
    static async verifyToken(token) {
        return (0, security_1.verifyJwtToken)(token);
    }
    /**
     ...
     */
    static async changePassword(userId, currentPassword, newPassword, forceChange = false) {
        try {
            if (!newPassword) {
                return {
            ...
            if (!forceChange) {
                ...
                const isCurrentPasswordValid = await (0, security_1.verifyPassword)(currentPassword, user.password);
            ...
            if (!(0, validators_1.validatePasswordStrength)(newPassword)) {
                return {
                    message: "Le nouveau mot de passe doit contenir au moins 8 caractères avec des majuscules, minuscules, chiffres et caractères spéciaux",
                    code: "PASSWORD_WEAK",
                };
            }
            const hashedNewPassword = await (0, security_1.hashPassword)(newPassword);
    ...
    static async checkPasswordChangeRequired(userId) {
        try {
            ...
            if (user.passwordChangedAt) {
                const daysSinceChange = Math.floor((Date.now() - user.passwordChangedAt.getTime()) /
                    (1000 * 60 * 60 * 24));
