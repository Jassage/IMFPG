...
class ClassService {
     ...
     */
    async getClasses(filters, auditData) {
        try {
            const { page = 1, limit = 20, search, level, academicYearId, status = "Active", sortBy = "name", sortOrder = "asc", } = filters;
     ...
     */
    async getClassById(id, auditData) {
        try {
     ...
     */
    async createClass(data, auditData) {
        try {
     ...
     */
    async updateClass(id, data, auditData) {
        try {
            ...
            if (name && name !== existingClass.name) {
                ...
                if (classWithName) {
                    return {
                        success: false,
                        message: "Une autre classe utilise déjà ce nom pour cette année académique",
                        code: "CLASS_NAME_EXISTS",
                    };
                }
            }
     ...
     */
    async deleteClass(id, auditData) {
        try {
            // Vérifier si la classe existe
            const schoolClass = await prisma.schoolClass.findUnique({
                where: { id },
            ...
            if (schoolClass._count.students > 0) {
                return {
                    success: false,
                    message: "Cette classe ne peut pas être supprimée car elle contient des élèves",
     ...
     */
    async getClassStats(id, auditData) {
        try {
            ...
            const statusCounts = stats.students.reduce((acc, student) => {
     ...
     */
    async getClassStudents(classId, auditData) {
        try {
     ...
     */
    async getClassSchedule(classId, auditData) {
        try {
     ...
     */
    async getClassLevels(auditData) {
        try {
     ...
     */
    async getOverallClassStats(auditData) {
        try {
            ...
            const classOccupancy = classStudentCounts.map((cls) => ({
                ...
                occupancyRate: cls.capacity > 0 ? (cls._count.students / cls.capacity) * 100 : 0,
            }));
