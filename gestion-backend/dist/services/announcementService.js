...
const getUserAudience = (userRole) => {
    if (!userRole)
        return "General";
    switch (userRole) {
        case "Admin":
...
const canViewAnnouncement = (announcement, userRole) => {
    if (!userRole)
        return false;
...
const canEditAnnouncement = (announcement, userId, userRole) => {
    if (!userId || !userRole)
        return false;
    ...
    if (["Secretaire", "Professeur"].includes(userRole) &&
...
class AnnouncementService {
    ...
    static async createAnnouncement(data) {
        const { title, content, publishDate, expiryDate, targetAudience = "All", priority = "Medium", attachments = [], userId, userRole, userEmail, } = data;
        ...
        if (!allowedRoles.includes(userRole)) {
            throw {
                status: 403,
                message: "Permission refusée. Seuls les administrateurs, directeurs, secrétaires et professeurs peuvent créer des annonces.",
            };
        }
    ...
    static async getAnnouncements(filters) {
        const { targetAudience, priority, isActive, startDate, endDate, page = "1", limit = "10", sortBy = "publishDate", sortOrder = "desc", search, authorId, userRole, } = filters;
     ...
     */
    static async getAnnouncementById(id, userRole) {
        ...
        if (!canViewAnnouncement(announcement, userRole)) {
            throw {
                status: 403,
                message: "Accès refusé. Vous n'avez pas les permissions nécessaires pour voir cette annonce.",
            };
        }
        return {
            success: true,
     ...
     */
    static async updateAnnouncement(id, updates, userData) {
        ...
        if (!canEditAnnouncement(existingAnnouncement, userId, userRole)) {
            throw {
                status: 403,
                message: "Permission refusée. Vous ne pouvez pas modifier cette annonce.",
            };
        }
     ...
     */
    static async deleteAnnouncement(id, userData) {
        const { userId, userRole, userEmail } = userData;
        // Vérification que l'utilisateur est authentifié
        if (!userId || !userRole) {
            throw {
        ...
        if (!allowedRoles.includes(userRole)) {
            throw {
                status: 403,
                message: "Permission refusée. Seuls les administrateurs et directeurs peuvent supprimer des annonces.",
            };
        }
        // Vérification de l'existence de l'annonce
        const existingAnnouncement = await prisma.announcement.findUnique({
     ...
     */
    static async deactivateAnnouncement(id, userData) {
        const { userId, userRole, userEmail } = userData;
        // Vérification que l'utilisateur est authentifié
        if (!userId || !userRole) {
            throw {
        ...
        if (!canEditAnnouncement(existingAnnouncement, userId, userRole)) {
            throw {
                status: 403,
                message: "Permission refusée. Vous ne pouvez pas désactiver cette annonce.",
            };
        }
     ...
     * Active une annonce
     */
    static async activateAnnouncement(id, userData) {
        const { userId, userRole, userEmail } = userData;
        // Vérification que l'utilisateur est authentifié
        if (!userId || !userRole) {
            throw {
        ...
        if (!canEditAnnouncement(existingAnnouncement, userId, userRole)) {
            throw {
                status: 403,
                message: "Permission refusée. Vous ne pouvez pas activer cette annonce.",
            };
        }
     ...
     */
    static async getAnnouncementStats(userRole) {
        // Vérification des permissions
        if (!userRole || !["Admin", "Directeur"].includes(userRole)) {
            throw {
                status: 403,
        ...
        const [totalAnnouncements, activeAnnouncements, expiredAnnouncements, announcementsByAudience, announcementsByPriority, announcementsByAuthor, recentAnnouncements,] = await Promise.all([
            // Total des annonces
        ...
        const statsByAudience = announcementsByAudience.reduce((acc, item) => {
            acc[item.targetAudience] = item._count;
            return acc;
        }, {});
        const statsByPriority = announcementsByPriority.reduce((acc, item) => {
