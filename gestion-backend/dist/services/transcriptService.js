...
class TranscriptService {
    ...
    normalizeGrade(grade, maxGrade) {
        return (grade / maxGrade) * 20;
    }
    // Calculer la note pondérée
    calculateWeightedGrade(normalizedGrade, coefficient) {
        return normalizedGrade * coefficient;
    }
    // Déterminer le statut de la note
    determineGradeStatus(grade, passingGrade, maxGrade) {
    ...
    async calculateStatistics(studentId, academicYearId, controlType, classLevel) {
        ...
        const allStudentsGrades = await prisma_1.default.grade.findMany({
            where: {
                academicYearId,
                controlType,
                classLevel: classLevel,
        ...
        const gradesWithCalculations = grades.map((grade) => {
            ...
            const normalizedGrade = this.normalizeGrade(grade.grade, subject.maxGrade);
            const weightedGrade = this.calculateWeightedGrade(normalizedGrade, subject.coefficient);
            const status = this.determineGradeStatus(grade.grade, subject.passingGrade, subject.maxGrade);
            return {
                id: grade.id,
                grade: grade.grade,
        ...
        const totalCoefficients = gradesWithCalculations.reduce((sum, grade) => sum + grade.coefficient, 0);
        const totalWeightedGrades = gradesWithCalculations.reduce((sum, grade) => sum + (grade.weightedGrade || 0), 0);
        const average = totalCoefficients > 0 ? totalWeightedGrades / totalCoefficients : 0;
        // Calculer les crédits
        const totalCredits = gradesWithCalculations.length * 3; // Exemple: 3 crédits par UE
        const creditsEarned = gradesWithCalculations.filter((g) => g.status === "PASSED").length * 3;
        // Calculer le taux de réussite
        const successRate = gradesWithCalculations.length > 0
        ...
        for (const [studentId, studentGrades] of studentGroups) {
            ...
            const totalCoeff = studentGradesWithCalc.reduce((sum, g) => sum + g.coefficient, 0);
            const totalWeighted = studentGradesWithCalc.reduce((sum, g) => sum + g.normalizedGrade * g.coefficient, 0);
    ...
    generateFileName(type, student, academicYear, controlType) {
        const typeMap = {
            BULLETIN: "bulletin",
            RELEVE: "releve",
    ...
    async getAllTranscripts(filter) {
        const { studentId, academicYearId, controlType, documentType, page = 1, limit = 10, } = filter;
        const where = {};
        if (studentId)
    ...
    async createTranscript(data) {
        const { studentId, academicYearId, controlType, classLevel, documentType, grades, statistics, language = prisma_2.DocumentLanguage.FR, withSignature = true, withStamp = true, } = data;
        // Validation
        if (!studentId || !academicYearId || !controlType || !classLevel) {
            throw new Error("Missing required fields: studentId, academicYearId, controlType, classLevel");
        }
        // Vérifier l'étudiant
        ...
        if (!calculatedStats) {
            calculatedStats = await this.calculateStatistics(studentId, academicYearId, controlType, classLevel);
        }
        // Générer le PDF
        const pdfBuffer = await (0, pdfService_1.generateDocumentPDF)({
            documentType,
            student,
            academicYear,
            controlType,
        ...
        const fileName = this.generateFileName(documentType, student, academicYear, controlType);
        ...
        const newTranscript = await prisma_1.default.transcript.create({
            data: {
                ...
                fileName,
                pdfData: pdfBuffer,
                status: prisma_2.TranscriptStatus.GENERATED,
                language,
                metadata: {
                    withSignature,
                    withStamp,
        ...
        // Créer un historique
        await prisma_1.default.documentHistory.create({
            data: {
    ...
    async updateTranscript(id, data) {
        ...
        if (data.controlType)
            updateData.controlType = data.controlType;
        if (data.classLevel)
    ...
    async downloadTranscript(id) {
        ...
        if (!pdfBuffer) {
            ...
            const stats = await this.calculateStatistics(transcript.studentId, transcript.academicYearId, transcript.controlType, transcript.classLevel);
            pdfBuffer = await (0, pdfService_1.generateDocumentPDF)({
