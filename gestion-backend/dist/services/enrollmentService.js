...
exports.EnrollmentService = exports.ClassLevel = void 0;
...
class EnrollmentService {
    ...
    async createEnrollment(data, auditData) {
        try {
            const { studentId, classId, academicYearId, enrollmentDate, assignFees = false, selectedFeeStructures = [], } = data;
            ...
            const enrollmentDateObj = enrollmentDate
                ...
                : new Date();
            const dateValidation = await this.validateEnrollmentDate(enrollmentDateObj, academicYearId);
    ...
    async updateEnrollment(id, data, auditData) {
        try {
            ...
            const modifiableValidation = await this.validateEnrollmentIsModifiable(enrollment);
            ...
            if (classId !== undefined && classId !== enrollment.classId) {
                ...
                const levelValidation = await this.validateClassLevelTransition(enrollment.studentId, classId, enrollment.classId, await this.getAcademicStatus(enrollment.studentId, enrollment.academicYearId));
            ...
            if (status !== undefined && status !== enrollment.status) {
                ...
                if (!validStatuses.includes(status)) {
                    return {
                        success: false,
                        message: "Statut invalide. Valeurs acceptées: Active, Suspended, Completed, Archived",
                        code: "INVALID_STATUS",
                    };
                }
                // Valider la transition de statut
                const statusValidation = await this.validateStatusTransition(enrollment.status, status, enrollment.id);
                ...
                if (status === "Completed") {
                    const canCompleteValidation = await this.validateCanCompleteEnrollment(enrollment.id);
     ...
     */
    async validateReenrollment(studentId, auditData) {
        try {
            // Vérifier si l'étudiant existe
            const student = await prisma.student.findUnique({
                where: { id: studentId },
            ...
            const financialValidation = await this.checkFinancialEligibility(studentId, previousEnrollment.academicYearId);
            ...
            const validation = {
                ...
                details: {
                    ...
                    recommendations: this.generateReenrollmentRecommendations(academicStatus, averageGrade, previousEnrollment.schoolClass.level, financialValidation),
                },
            };
            return {
                success: true,
    ...
    async reenrollStudent(data, auditData) {
        try {
            const { studentId, classId, academicYearId, previousAcademicYearId, previousEnrollmentId, enrollmentDate, notes, } = data;
            ...
            if (previousEnrollmentId) {
                ...
                console.log("Recherche de l'inscription precedente specifiee:", previousEnrollmentId);
                ...
                if (!previousEnrollment) {
                    console.error("Inscription precedente non trouvee:", previousEnrollmentId);
                    return {
                        success: false,
            ...
            if (previousYear.endDate >= targetYear.startDate) {
                return {
                    success: false,
                    message: "L'année précédente doit se terminer avant la nouvelle année",
            ...
            const existingEnrollmentForTargetYear = await prisma.enrollment.findUnique({
                where: {
                    studentId_academicYearId: {
                        studentId,
            ...
            console.log("Classe trouvee:", schoolClass.name, "- Niveau:", schoolClass.level);
            // 10. Déterminer le statut académique pour validation des niveaux
            const academicStatus = await this.getAcademicStatus(studentId, previousYear.id);
            // 11. Valider la transition de niveau (avec support du redoublement)
            const levelValidation = await this.validateClassLevelTransition(studentId, classId, previousEnrollment.classId, academicStatus);
            if (!levelValidation.valid) {
                return {
                    success: false,
                    message: levelValidation.reason,
            ...
            const financialValidation = await this.checkFinancialEligibility(studentId, previousYear.id);
            ...
            const chainValidation = await this.validateEnrollmentChain("new-enrollment", previousEnrollment.id);
    ...
    async createBulkEnrollments(enrollments, auditData) {
        try {
            ...
            for (const enrollmentData of enrollments) {
                try {
                    const { studentId, classId, academicYearId, enrollmentDate } = enrollmentData;
                    // Vérifications
                    const student = await prisma.student.findUnique({
                        where: { id: studentId },
                    });
     ...
     */
    async deleteEnrollment(id, auditData) {
        try {
            // Vérifier si l'inscription existe
            const enrollment = await prisma.enrollment.findUnique({
                where: { id },
            ...
            if (enrollment.status === "Completed") {
                return {
                    success: false,
                    message: "Une inscription terminée ou archivée ne peut pas être supprimée",
            ...
            if (nextEnrollments.length > 0) {
                return {
                    success: false,
                    message: "Cette inscription a des inscriptions suivantes et ne peut pas être supprimée",
     ...
     */
    async getStudentEnrollmentHistory(studentId, auditData) {
        try {
            const enrollments = await prisma.enrollment.findMany({
                where: { studentId },
                include: {
            ...
            console.error("EnrollmentService - getStudentEnrollmentHistory error:", error);
            throw error;
        }
    }
    ...
    async calculateReenrollmentFee(studentId, previousYearId, currentYearId) {
        try {
            ...
            if (currentAcademicYear) {
                ...
                estimatedYearFees = currentYearFeeStructures.reduce((sum, fee) => sum + fee.amount, 0);
                console.log(`Frais estimes pour ${currentAcademicYear.year}:`, estimatedYearFees);
            }
            else {
     ...
     * @private
     */
    async createStudentFees(studentId, academicYearId, tx) {
        ...
        try {
            ...
            if (feeStructures.length === 0) {
                console.log(`Aucune structure de frais active trouvee pour ${academicYearName}`);
            ...
            const createPromises = feeStructures.map(async (feeStructure) => {
                ...
                console.log(`Frais crees: ${feeStructure.name} - ${feeStructure.amount} HTG`);
                return studentFee;
            });
     ...
     * @private
     */
    validateAcademicResults(grades) {
        if (grades.length === 0)
            return false;
     ...
     * @private
     */
    async getAcademicStatus(studentId, academicYearId) {
        try {
            const grades = await prisma.grade.findMany({
                where: {
                    studentId,
     ...
     * @private
     */
    async validateEnrollmentDate(enrollmentDate, academicYearId) {
        try {
            const academicYear = await prisma.academicYear.findUnique({
                where: { id: academicYearId },
            });
            ...
            if (enrollmentDate < academicYear.startDate) {
                return {
                    valid: false,
                    reason: "La date d'inscription ne peut pas être antérieure au début de l'année académique",
                };
            }
            if (enrollmentDate > academicYear.endDate) {
                return {
                    valid: false,
                    reason: "La date d'inscription ne peut pas être postérieure à la fin de l'année académique",
                };
            }
            return { valid: true };
        }
     ...
     * @private
     */
    async validateStatusTransition(oldStatus, newStatus, enrollmentId) {
        ...
        if (newStatus === "Completed") {
            ...
            if (enrollment && enrollment.academicYear) {
                ...
                const daysUntilEnd = Math.ceil((enrollment.academicYear.endDate.getTime() - today.getTime()) /
                    (1000 * 3600 * 24));
                if (daysUntilEnd > 30) {
                    return {
                        valid: false,
     ...
     * @private
     */
    async validateCanCompleteEnrollment(enrollmentId) {
        try {
            const enrollment = await prisma.enrollment.findUnique({
                where: { id: enrollmentId },
                include: {
            ...
            const financialValidation = await this.checkFinancialEligibility(enrollment.studentId, enrollment.academicYearId);
            if (!financialValidation.eligible) {
                return {
                    valid: false,
                ...
                if (!passed) {
                    return {
                        ...
                        details: {
                            ...
                            averageGrade: grades.reduce((sum, grade) => sum + grade.grade, 0) /
                                grades.length,
                        },
                    };
                }
     ...
     * @private
     */
    async checkFinancialEligibility(studentId, academicYearId) {
        try {
     ...
     * @private
     */
    getAllowedNextLevels(currentLevel, academicStatus = "Passed") {
        const nextLevels = [];
     ...
     * @private
     */
    async validateClassLevelTransition(studentId, newClassId, previousClassId, academicStatus = "Passed") {
        if (!previousClassId) {
        ...
        try {
            ...
            if (newLevel === previousLevel) {
                const canRepeat = await this.validateLevelRepetition(studentId, previousClassId);
                if (!canRepeat.valid) {
                    return {
                        valid: false,
            ...
            const rules = {
                ...
                fromQuatrieme: previousLevel === ClassLevel.Quatrieme &&
            ...
            if (rules.goDownLevel) {
                return {
                    ...
                    details: {
                        from: previousLevel,
                        to: newLevel,
                        allowedNext: this.getAllowedNextLevels(previousLevel, academicStatus),
                    },
                };
            }
            if (rules.skipLevel) {
                ...
                if (academicStatus === "Passed") {
                    const grades = await prisma.grade.findMany({
                        where: {
                            studentId,
                            academicYear: {
                                id: (await prisma.enrollment.findFirst({
                                    where: {
                                        studentId,
                    ...
                    if (average < 80) {
                        return {
                            valid: false,
                            reason: "Le saut de niveau n'est autorisé que pour les élèves avec une moyenne supérieure à 80",
                            details: {
                                average,
                                requiredAverage: 80,
                            },
            ...
            if (previousLevel === ClassLevel.Quatrieme) {
                if (!rules.fromQuatrieme) {
                    return {
                        valid: false,
                        reason: "Après la Quatrième, l'étudiant doit aller en Troisième (parcours classique) ou en NSI (Nouveau Secondaire)",
                        details: {
                ...
                if (newLevel === ClassLevel.NSI) {
                    const orientationValid = await this.validateNouveauSecondaireOrientation(studentId);
                    if (!orientationValid.valid) {
                        return {
                            valid: false,
                            reason: orientationValid.reason ||
            ...
            if (!validTransition) {
                return {
                    ...
                    details: {
                        from: previousLevel,
                        to: newLevel,
                        allowedTransitions: this.getAllowedTransitions(previousLevel, academicStatus),
                    },
                };
            }
            return { valid: true };
     ...
     * @private
     */
    async validateNouveauSecondaireOrientation(studentId) {
        try {
            ...
            const grades = await prisma.grade.findMany({
                where: {
                    studentId,
                    academicYearId: academicYearIds.length > 0 ? { in: academicYearIds } : undefined,
                },
                include: {
                    subject: true,
                },
            ...
            const mathGrade = grades.find((g) => (g.subject?.name ?? "").toLowerCase().includes("math"))?.grade || 0;
            const scienceGrade = grades.find((g) => (g.subject?.name ?? "").toLowerCase().includes("science") ||
                (g.subject?.name ?? "").toLowerCase().includes("physique") ||
                (g.subject?.name ?? "").toLowerCase().includes("chimie"))?.grade || 0;
            // Règles d'orientation
            const meetsCriteria = average >= 60 && mathGrade >= 50 && scienceGrade >= 50;
            if (!meetsCriteria) {
                return {
                    valid: false,
                    reason: "L'étudiant ne remplit pas les critères pour le Nouveau Secondaire",
                    details: {
                        average,
                        mathGrade,
                        scienceGrade,
     ...
     * @private
     */
    async validateLevelRepetition(studentId, classId) {
        try {
            ...
            if (enrollments.length > 0) {
                ...
                if (grades.length > 0) {
                    ...
                    if (passed) {
                        return {
                            valid: false,
                            reason: "L'étudiant a déjà réussi ce niveau, le redoublement n'est pas nécessaire",
                            details: {
                                average,
                                passed: true,
                            },
     ...
     * @private
     */
    getAllowedTransitions(currentLevel, academicStatus = "Passed") {
        const transitions = [];
        // Toujours permettre le même niveau (redoublement)
        transitions.push({
            to: currentLevel,
            description: academicStatus === "Failed"
     ...
     * @private
     */
    generateReenrollmentRecommendations(academicStatus, averageGrade, previousLevel, financialValidation) {
        ...
        if (academicStatus === "Failed") {
            ...
            if (averageGrade < 30) {
                recommendations.push({
                    type: "error",
                    message: "Résultats très faibles - Recommander une évaluation pédagogique",
        ...
        const levelRecommendations = this.getLevelRecommendations(previousLevel, academicStatus);
     ...
     * @private
     */
    getLevelRecommendations(previousLevel, academicStatus) {
        const recommendations = [];
        if (academicStatus === "Failed") {
            if ([ClassLevel.Terminale, ClassLevel.NSIV].includes(previousLevel)) {
                recommendations.push({
                    type: "error",
        ...
        if (previousLevel === ClassLevel.Quatrieme && academicStatus === "Passed") {
            recommendations.push({
                ...
                action: "Évaluer les options: Troisième (classique) ou NSI (Nouveau Secondaire)",
            });
        }
        return recommendations;
    }
     ...
     * @private
     */
    async validateEnrollmentChain(enrollmentId, previousEnrollmentId) {
        const errors = [];
        if (previousEnrollmentId) {
            ...
            if (!previous) {
                errors.push(`L'inscription précédente ${previousEnrollmentId} n'existe pas`);
            }
            else {
                ...
                if (currentEnrollment && currentEnrollment.academicYear) {
                    if (previous.academicYear.endDate >=
                        currentEnrollment.academicYear.startDate) {
                        errors.push("L'année précédente doit se terminer avant la nouvelle année");
                    }
                }
     ...
     * @private
     */
    async hasCycle(startId, targetId) {
     ...
     * @private
     */
    async validateEnrollmentIsModifiable(enrollment) {
        ...
        if (enrollment.academicYear) {
            ...
            if (today > enrollment.academicYear.endDate) {
                return {
                    valid: false,
                    reason: "L'année académique est terminée, l'inscription n'est plus modifiable",
                };
            }
        }
        return { valid: true };
     ...
     */
    async manualHistoricalReenrollment(data, auditData) {
        try {
            ...
            if (data.targetAcademicYearId === data.previousAcademicYearId) {
                return {
                    success: false,
                    message: "L'année cible et l'année précédente doivent être différentes",
                    code: "SAME_YEARS",
                };
            }
            ...
            if (result.success &&
                ...
                if (enrollmentId) {
                    await this.assignFeesOnEnrollment(data.studentId, data.targetAcademicYearId, data.selectedFeeStructures);
                    return {
                        ...result,
            ...
            console.error("EnrollmentService - manualHistoricalReenrollment error:", error);
            throw error;
        }
    }
}
