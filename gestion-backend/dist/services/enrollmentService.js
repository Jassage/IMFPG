...
class EnrollmentService {
     ...
     */
    async getStudentEnrollmentYears(studentId) {
            ...
            const validation = {
                ...
                details: {
                    ...
                    financial: {
                        ...
                        overdueAmount: overdueFees.reduce((sum, fee) => sum + (fee.totalAmount - fee.paidAmount), 0),
    ...
    async calculateReenrollmentFee(studentId, previousYearId, currentYearId) {
        try {
            ...
            if (currentAcademicYear) {
                ...
                console.log(`📊 Frais estimés pour ${currentAcademicYear.year}:`, estimatedYearFees);
            }
            else {
                console.warn("⚠️ Année académique courante non trouvée:", currentYearId);
            }
            // 6. Calculer le total
    ...
    async createStudentFees(studentId, academicYearId, tx) {
        ...
        try {
            ...
            if (feeStructures.length === 0) {
                console.log(`⚠️ Aucune structure de frais active trouvée pour ${academicYearName}`);
    ...
    async reenrollStudent(data, auditData) {
        try {
            const { studentId, classId, academicYearId, // C'est l'année TARGET (nouvelle année)
            previousAcademicYearId, // Année précédente (optionnelle)
            enrollmentDate, notes, } = data;
            ...
            if (previousAcademicYearId) {
                ...
                console.log("📌 Recherche de l'année précédente spécifiée:", previousAcademicYearId);
                previousYear = await prisma.academicYear.findUnique({
                    where: { id: previousAcademicYearId },
                });
                if (!previousYear) {
                    console.error("❌ Année précédente spécifiée non trouvée:", previousAcademicYearId);
                    return {
                        success: false,
                ...
                console.log("✅ Inscription précédente trouvée (spécifiée):", previousEnrollment.id);
            }
            else {
                ...
                if (previousYear) {
                    console.log("📅 Année précédente trouvée par date:", previousYear.year);
                    previousEnrollment = await prisma.enrollment.findUnique({
                        where: {
                            studentId_academicYearId: {
                                studentId,
                    ...
                    if (!previousEnrollment) {
                        console.log("⚠️ Pas d'inscription pour l'année trouvée par date, tentative alternative...");
                    }
                }
                // Stratégie 2: Si pas trouvé, chercher la dernière inscription de l'étudiant
                if (!previousEnrollment) {
                    console.log("🔍 Recherche de la dernière inscription de l'étudiant...");
                ...
                if (!previousEnrollment) {
                    console.log("🔍 Recherche de n'importe quelle inscription précédente...");
                    ...
                    if (allPreviousEnrollments.length > 0) {
                        ...
                        previousYear = previousEnrollment.academicYear;
                        console.log("✅ Inscription historique trouvée:", previousYear.year);
                    }
                }
            }
            ...
            if (!previousEnrollment || !previousYear) {
                ...
                return {
                    success: false,
                    message: "Aucune inscription précédente trouvée. Veuillez spécifier l'année précédente.",
                    code: "NO_PREVIOUS_ENROLLMENT",
                    data: {
