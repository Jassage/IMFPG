"use strict";
/**
 * @file scheduleRoutes.ts
 * @description Routes pour la gestion des emplois du temps
 * @version 1.0.0
 */
Object.defineProperty(exports, "__esModule", { value: true });
const express_1 = require("express");
const middleware_1 = require("../middleware");
const scheduleController_1 = require("../controllers/scheduleController");
const scheduleValidators_1 = require("../utils/scheduleValidators");
const router = (0, express_1.Router)();
/**
 * @route GET /api/academic/schedules
 * @description Récupère tous les horaires avec pagination et filtres
 * @access Staff/Admin
 * @query {number} [page=1] - Numéro de page
 * @query {number} [limit=20] - Nombre d'éléments par page
 * @query {string} [classId] - ID de la classe
 * @query {string} [professeurId] - ID du professeur
 * @query {string} [dayOfWeek] - Jour de la semaine
 * @query {string} [status] - Statut (ACTIVE/INACTIVE/CANCELLED)
 * @query {string} [academicYearId] - ID de l'année académique
 */
router.get("/", middleware_1.requireAuth, middleware_1.requireStaff, scheduleController_1.getAllSchedules);
/**
 * @route GET /api/academic/schedules/:id
 * @description Récupère un horaire par ID avec ses relations
 * @access Staff/Admin
 * @param {string} id - ID de l'horaire
 */
router.get("/:id", middleware_1.requireAuth, middleware_1.requireStaff, middleware_1.sanitizeInput, scheduleController_1.getScheduleById);
/**
 * @route GET /api/academic/schedules/class/:classId
 * @description Récupère l'emploi du temps d'une classe
 * @access Staff/Admin
 * @param {string} classId - ID de la classe
 * @query {string} academicYearId - ID de l'année académique
 * @query {string} [startDate] - Date de début (optionnel)
 * @query {string} [endDate] - Date de fin (optionnel)
 */
router.get("/class/:classId", middleware_1.requireAuth, middleware_1.sanitizeInput, scheduleController_1.getClassTimetable);
/**
 * @route GET /api/academic/schedules/professor/:professeurId
 * @description Récupère l'emploi du temps d'un professeur
 * @access Staff/Admin/Professeur
 * @param {string} professeurId - ID du professeur
 * @query {string} [startDate] - Date de début (optionnel)
 * @query {string} [endDate] - Date de fin (optionnel)
 */
router.get("/professor/:professeurId", middleware_1.requireAuth, middleware_1.sanitizeInput, scheduleController_1.getProfessorSchedule);
/**
 * @route GET /api/academic/schedules/available-slots
 * @description Récupère les créneaux disponibles
 * @access Staff/Admin
 * @query {string} classId - ID de la classe
 * @query {string} dayOfWeek - Jour de la semaine
 * @query {string} [professeurId] - ID du professeur (optionnel)
 * @query {string} [classroom] - Salle (optionnel)
 */
router.get("/available-slots", middleware_1.requireAuth, middleware_1.requireStaff, middleware_1.sanitizeInput, scheduleController_1.getAvailableTimeSlots);
/**
 * @route GET /api/academic/schedules/check-conflicts
 * @description Vérifie les conflits d'horaire
 * @access Staff/Admin
 * @query {string} professeurId - ID du professeur
 * @query {string} classId - ID de la classe
 * @query {string} dayOfWeek - Jour de la semaine
 * @query {string} startTime - Heure de début
 * @query {string} endTime - Heure de fin
 * @query {string} [classroom] - Salle (optionnel)
 * @query {string} [excludeScheduleId] - ID de l'horaire à exclure (optionnel)
 */
router.get("/check-conflicts", middleware_1.requireAuth, middleware_1.requireStaff, middleware_1.sanitizeInput, scheduleController_1.checkConflicts);
/**
 * @route POST /api/academic/schedules
 * @description Crée un nouvel horaire
 * @access Admin/Staff
 * @body {string} assignmentId - ID de l'assignation
 * @body {string} classId - ID de la classe
 * @body {string} dayOfWeek - Jour de la semaine (MONDAY, TUESDAY, etc.)
 * @body {string} startTime - Heure de début (format: HH:mm)
 * @body {string} endTime - Heure de fin (format: HH:mm)
 * @body {string} [classroom] - Salle de classe
 * @body {string} [recurrence] - Récurrence (WEEKLY, BIWEEKLY, MONTHLY)
 * @body {string} [untilDate] - Date de fin de récurrence
 * @body {string} [notes] - Notes supplémentaires
 */
router.post("/", middleware_1.requireAuth, middleware_1.requireStaff, (0, middleware_1.validateContentType)(), middleware_1.validateRequestBody, middleware_1.sanitizeInput, scheduleValidators_1.validateCreateSchedule, middleware_1.handleValidationErrors, scheduleController_1.createSchedule);
/**
 * @route POST /api/academic/schedules/generate
 * @description Génère un emploi du temps automatiquement
 * @access Admin
 * @body {string} classId - ID de la classe
 * @body {string} academicYearId - ID de l'année académique
 * @body {Object} [constraints] - Contraintes de génération
 * @body {number} [constraints.maxHoursPerDay=6] - Heures max par jour
 * @body {Object} [constraints.breakTime] - Pause déjeuner
 * @body {string} [constraints.breakTime.start] - Début pause
 * @body {string} [constraints.breakTime.end] - Fin pause
 */
router.post("/generate", middleware_1.requireAuth, middleware_1.requireAdmin, (0, middleware_1.validateContentType)(), middleware_1.validateRequestBody, middleware_1.sanitizeInput, scheduleValidators_1.validateGenerateTimetable, middleware_1.handleValidationErrors, scheduleController_1.generateTimetable);
/**
 * @route PUT /api/academic/schedules/:id
 * @description Met à jour un horaire
 * @access Admin/Staff
 * @param {string} id - ID de l'horaire
 * @body {string} [dayOfWeek] - Jour de la semaine
 * @body {string} [startTime] - Heure de début
 * @body {string} [endTime] - Heure de fin
 * @body {string} [classroom] - Salle de classe
 * @body {string} [recurrence] - Récurrence
 * @body {string} [untilDate] - Date de fin de récurrence
 * @body {string} [notes] - Notes
 * @body {string} [status] - Statut (ACTIVE/INACTIVE/CANCELLED)
 */
router.put("/:id", middleware_1.requireAuth, middleware_1.requireStaff, (0, middleware_1.validateContentType)(), middleware_1.validateRequestBody, middleware_1.sanitizeInput, scheduleValidators_1.validateUpdateSchedule, middleware_1.handleValidationErrors, scheduleController_1.updateSchedule);
/**
 * @route DELETE /api/academic/schedules/:id
 * @description Supprime un horaire
 * @access Admin
 * @param {string} id - ID de l'horaire
 */
router.delete("/:id", middleware_1.requireAuth, middleware_1.requireAdmin, middleware_1.sanitizeInput, scheduleController_1.deleteSchedule);
exports.default = router;
//# sourceMappingURL=scheduleRoutes.js.map